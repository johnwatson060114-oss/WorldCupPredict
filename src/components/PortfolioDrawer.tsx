    ...
    <div className="drawer-backdrop" onMouseDown={onClose}>
      <aside className="portfolio-drawer" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="drawer-tickets">
          {portfolio.tickets.map((ticket) => <article key={ticket.id}><div><span>{ticket.type}</span><strong>{ticket.stake}元</strong></div>{ticket.legs.map((leg) => <p key={`${ticket.id}-${leg.matchId}-${leg.selection}`}>{leg.label} · {leg.market} {leg.selection} @{leg.odds.toFixed(2)}</p>)}<footer><span>组合奖金 {ticket.combinedOdds.toFixed(2)}</span><span>稳健期望 {signedPercent(ticket.robustExpectedReturn)}</span><span>最高返还 {ticket.potentialPayout.toFixed(0)}元</span></footer></article>)}
        </div>
